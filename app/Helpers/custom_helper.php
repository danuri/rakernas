...
if ( ! function_exists('get_option'))
{
    ...
    $db = \Config\Database::connect('default', false);
		$query = $db->query("SELECT option_value FROM options WHERE option_name='$name'")->getRow();
...
if ( ! function_exists('update_option'))
{
	...
	{
		$db = \Config\Database::connect('default', false);
    $db->query("UPDATE options SET option_value='$value' WHERE option_name='$name'");
...
{
  if($status == 0){
    $result = '<span class="badge bg-success">Usul Masuk</span>';
  }else if($status == 1){
    ...
    $result = '<span class="badge bg-info">Menunggu TTD SK</span>';
  }else if($status == 9){
